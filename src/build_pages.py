...
def build_pages():
    ...
    with open('README.md', 'w') as f:
        ...
        f.write("Newest first.\n\n")
        # One collapsed section per topic so the list doesn't swamp the README
        for category in ordered_categories:
            cat_papers = papers_by_category[category]
            # The topic name links to its page on the site; the rest of the
            # summary line toggles the list
            url = f"{SITE_URL}{listing_url(topic_folder(category))}"
            f.write(f'<details>\n<summary><b><a href="{url}">{category}</a></b> '
                    f'({len(cat_papers)})</summary>\n\n')
            for paper in cat_papers:
                f.write(f"- **{paper['title']}** ({paper_month(paper)}) - "
                        f"[arXiv:{base_id(paper)}](https://arxiv.org/abs/{paper['arxiv']})\n")
            f.write('\n</details>\n\n')
